...
+---------------------------+------------------------------------------------------------+
| Madeleine Ekstrand        | Same as above                                              |
+---------------------------+------------------------------------------------------------+
| Yassin Hjuler El Mahdaoui | Same as above                                              |
+---------------------------+------------------------------------------------------------+
...
+=====================+===============+=============+==================+===================+
| **intercept**       | 5.4953516236  | 192.5693633 | 102.8209510      | 360.6556770       |
+---------------------+---------------+-------------+------------------+-------------------+
...
+---------------------+---------------+-------------+------------------+-------------------+
| **smokstatformer**  | -0.0841591231 | 0.9192850   | 0.7799284        | 1.0835416         |
+---------------------+---------------+-------------+------------------+-------------------+
| **smokstatcurrent** | -0.2936459897 | 0.7455404   | 0.5867189        | 0.9473540         |
+---------------------+---------------+-------------+------------------+-------------------+
| **sex**             | 0.2348952060  | 1.2647762   | 0.9890707        | 1.6173352         |
+---------------------+---------------+-------------+------------------+-------------------+
| **vituse\           | -0.0210617860 | 0.9791585   | 0.8094944        | 1.1843828         |
| not often**         |               |             |                  |                   |
+---------------------+---------------+-------------+------------------+-------------------+
| **vituse\           | -0.2817784870 | 0.7544408   | 0.6318927        | 0.9007556         |
| no**                |               |             |                  |                   |
+---------------------+---------------+-------------+------------------+-------------------+

: *Reestimated coefficients for model 3.3.1 on the reduced dataset*
...
no_outlier <- filter(study_data, cholesterol < max(study_data$cholesterol))

new_model3c <- update(model_3c, data = no_outlier)
...
f1.new_model <- pplus1
f2.new_model <- new_model3c$df.residual
cook.limit.new_model <- qf(0.5, f1.new_model, f2.new_model)
n_new = nobs(new_model3c)

ggplot(no_outlier, aes(yhat_new, D_new)) + 
  geom_point(size = 3) +
  geom_hline(yintercept = cook.limit.new_model, color = "red") +
  geom_hline(yintercept = 4/n_new, linetype = 2, color = "red") +
  xlab("Fitted values (log β-carotene)") +
  ylab("Cook's D") +
  labs(title = "Model 3(c): Cook's Distance vs Fitted Values - refitted on reduced dataset",
       caption = "4/n (dashed), F_0.5, p+1, n-(p+1) (solid)",
       color = "Highlight") +
  scale_color_manual(values = highlightcolors)
```

*Cook's distance plotted against fitted values for model 3.3.1 (model 3.3 reestimated on reduced dataset)*
...
We performed an stepwise variable selection for **model 3.3** and **model 3.3.1** with BIC as criterion, for which the only difference is the dataset used to estimate these models. For both of these, we started with the null model as the smallest (which is just the intercept), and then the largest allowed model being all variables seen in the table in *Chapter 4.1.* For the two models, the variables added were **different**. In neither of the two case were any variables removed after being added (no backward steps). The selection order for the model with outlier in the data was:

-   *bmi* $\rightarrow$ *fiber* $\rightarrow$ *cholesterol* $\rightarrow$ *vituse_no* and *vituse_not_often*

And for the model without outlier in the data:

-   *bmi* $\rightarrow$ *fiber* $\rightarrow$ *fat* $\rightarrow$ *vituse_no* and \*vituse_not_often\
    -   

The parameter estimates and confidence intervals for in the case of the full data set as well as the reduced dataset are seen below (we call these models 4b). The interesting thing is that the variables that differs between the models are **cholesterol** and **fat**. As seen in *3.2* and *4.1*, these variables had high correlation, so it is not a surprise that either model only includes one of these. Also interesting is that in the case without outlier, the stepwise selection chose fat instead of cholesterol. This also makes sense, as we found that the the outlier was an outlier exactly from the extreme value in cholesterol.
